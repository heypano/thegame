...
{
	if(spaceShipLife>=LIFEQUANT){
		var lasso:TheLasso = new TheLasso(currentMovingObject.x, currentMovingObject.y - 10, currentMovingObject.rotation);
		lassoContainer.addChild(lasso);
		lasso.initialize();
		spaceShipLife-=LIFEQUANT;
		adjustLifeBar();
...
public function stringToBinary(str:String):String{
	var num:int;
	num = int(str);
	return num.toString(2);
}
public function getBinaryDigiti(str:String,i:int):int{ //i is counting from left to right 1 being : 12345678 e.g. getBinaryDigiti("2",7) is 1 and every other i is 0
	var num:int;
	var mask:int=1;
	i=9-i;
	num = int(str);
...
public function setBinaryDigiti(str:String,i:int,to:int):int{ //i is counting from left to right 1 being : 12345678
	var num:int;
	var mask:int=1;
	i=9-i;
	num = int(str);
